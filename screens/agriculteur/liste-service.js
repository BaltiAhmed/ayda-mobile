...
const ListeService = (props) => {
  ...
  const onRefresh = useCallback(() => {
    ...
    const sendRequest = async () => {
      const response = await fetch(
        `http://192.168.43.177:5000/api/service`
      );
  ...
  useEffect(() => {
    const sendRequest = async () => {
      const response = await fetch(`http://192.168.43.177:5000/api/service`);
  ...
  const submit = async () => {
    ...
    let response = await fetch("http://192.168.43.177:5000/api/demandeService/ajoutDemandeService", {
      method: "POST",
      headers: {
